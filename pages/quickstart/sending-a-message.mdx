...
* [DIVE Tutorial: Setting up a local environment with DIVE cli](https://dive-3.gitbook.io/dive-v2/).
...
When the `xcall` contract on the source chain receives the call request message, it sends the `_data` to the [BTP address](https://docs.icon.community/cross-chain-communication/blockchain-transmission-protocol-btp/btp-address) `_to` on the destination chain.
...
* The `_to` param is a [BTP formatted address](https://docs.icon.community/cross-chain-communication/blockchain-transmission-protocol-btp/btp-address) of the callee contract which receives the `_data` payload on the destination chain.
